...
async function deploy(siteDir) {
    ...
    if (!organizationName) {
        throw new Error(`Missing project organization name. Did you forget to define 'organizationName' in ${constants_1.CONFIG_FILE_NAME}? You may also export it via the organizationName environment variable.`);
    }
    const projectName = process.env.PROJECT_NAME ||
        process.env.CIRCLE_PROJECT_REPONAME ||
        siteConfig.projectName;
    if (!projectName) {
        throw new Error(`Missing project name. Did you forget to define 'projectName' in ${constants_1.CONFIG_FILE_NAME}? You may also export it via the projectName environment variable.`);
    }
    ...
    const deploymentBranch = projectName.indexOf('.github.io') !== -1 ? 'master' : 'gh-pages';
    ...
    if (currentBranch === deploymentBranch && !crossRepoPublish) {
        throw new Error(`Cannot deploy from a ${deploymentBranch} branch. Only to it`);
    }
    ...
    const tempDir = path_1.default.join(siteDir, '.docusaurus');
    fs_extra_1.default.removeSync(tempDir);
    // Build static html files, then push to deploymentBranch branch of specified repo.
    build_1.build(siteDir)
        .then(() => {
        shelljs_1.default.cd(tempDir);
        if (shelljs_1.default.exec(`git clone ${remoteBranch} ${projectName}-${deploymentBranch}`).code !== 0) {
        ...
        if (defaultBranch !== deploymentBranch) {
            if (shelljs_1.default.exec(`git checkout origin/${deploymentBranch}`).code !== 0) {
                ...
                shelljs_1.default.exec(`git branch --set-upstream-to=origin/${deploymentBranch}`).code !==
                0) {
                throw new Error(`Error: Git checkout ${deploymentBranch} failed`);
            }
        }
        ...
        const fromPath = path_1.default.join(constants_1.BUILD_DIR_NAME);
        ...
        fs_extra_1.default.copy(fromPath, toPath, error => {
            ...
            if (shelljs_1.default.exec(`git push --force origin ${deploymentBranch}`).code !== 0) {
                ...
                const websiteURL = githubHost === 'github.com'
